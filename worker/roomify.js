...
/* ==========================================================================
 * SCRATCH: spec 0011's task 1 platform check. DELETE THIS WHOLE BLOCK once the
 * three answers are recorded in the spec's Follow-up section.
 *
 * It exists to prove, against the real runtime rather than against the docs,
 * the three facts the flat feed index rests on:
 *
 *   1. that the app scoped `me.puter` exposes `kv.set`, `kv.del` and `kv.list`,
 *      and that `list` takes `limit` and `cursor` and can return values;
 *   2. that a listed page comes back in KEY ORDER, and stays in key order
 *      across a cursor boundary;
 *   3. whether that cursor is opaque (it remembers the last key it handed out)
 *      or positional (it remembers an offset), which decides what happens when
 *      a key is deleted between two pages.
 *
 * Every route is `/kv-probe/*` and every key it writes is under `probe:`, so
 * nothing here can touch a real feed key even by accident. It is deliberately
 * NOT behind a session check: it writes only its own prefix in the app's own
 * store, and needing a token would make it far harder to run with `curl`.
 * That is also the reason it has to be deleted rather than left in place.
 * ========================================================================== */

const PROBE_PREFIX = "probe:entry:";
const PROBE_PATTERN = "probe:entry:*";

/** The app scoped Puter, under whichever name this runtime actually injects it. */
const appPuter = () => {
  // Both spellings, because which one this runtime injects is part of what the
  // probe is here to find out. `me` may be a true global or a binding in the
  // wrapper scope, so it is read directly as well as off `globalThis`.
  // eslint-disable-next-line no-undef
  const injected = typeof me !== "undefined" ? me : globalThis.me;
  return injected && injected.puter ? injected.puter : null;
};

/** `probe:entry:0007`, so plain string order is also numeric order. */
const probeKey = (n) => `${PROBE_PREFIX}${String(n).padStart(4, "0")}`;

/** Fact 1: what does this runtime actually give us? */
router.get("/kv-probe/surface", async () => {
  const puter = appPuter();
  if (!puter) {
    return json(
      { ok: false, note: "No app scoped puter found under `me` or `self.me`." },
      500,
    );
  }
  const kv = puter.kv ?? {};
  return json(
    {
      ok: true,
      hasKv: Boolean(puter.kv),
      methods: Object.keys(kv).concat(
        ["set", "get", "del", "list", "incr"].filter(
          (name) => typeof kv[name] === "function",
        ),
      ),
      listIsFunction: typeof kv.list === "function",
    },
    200,
  );
});

/** Writes `count` keys (default 30) so there is enough to need two pages. */
router.post("/kv-probe/seed", async ({ request }) => {
  const puter = appPuter();
  if (!puter) return json({ ok: false, note: "no app puter" }, 500);

  const count = Number(new URL(request.url).searchParams.get("count") ?? "30");
  if (!Number.isInteger(count) || count < 1 || count > 200) {
    return json({ ok: false, note: "count must be 1 to 200" }, 400);
  }

  // Written in a deliberately SHUFFLED order, so that a page coming back sorted
  // proves the store sorts by key rather than that it happens to return things
  // in the order they were inserted.
  const order = Array.from({ length: count }, (_, i) => i + 1).sort(
    () => Math.random() - 0.5,
  );
  for (const n of order) {
    await puter.kv.set(probeKey(n), { n, note: `probe entry ${n}` });
  }
  return json({ ok: true, wrote: count, writtenInOrder: order }, 200);
});

/** Facts 2 and 3: one page, with an optional cursor. */
router.get("/kv-probe/page", async ({ request }) => {
  const puter = appPuter();
  if (!puter) return json({ ok: false, note: "no app puter" }, 500);
  ...
  try {
    ...
    // The shape of what `list` returns is itself one of the unknowns, so the
    // raw answer is echoed alongside our reading of it rather than instead of it.
    const items = Array.isArray(page)
      ? page
      : (page?.items ?? page?.keys ?? []);
    const keys = items.map((item) =>
      typeof item === "string" ? item : (item?.key ?? null),
    );

    return json(
      {
        ok: true,
        rawShape: Array.isArray(page) ? "array" : Object.keys(page ?? {}),
        count: keys.length,
        keys,
        inKeyOrder: keys.every(
          (key, i) => i === 0 || String(keys[i - 1]) <= String(key),
        ),
        nextCursor: Array.isArray(page) ? null : (page?.cursor ?? null),
      },
      200,
    );
  } catch (error) {
    return json({ ok: false, note: String(error?.message ?? error) }, 500);
  }
});

/** Deletes one probe key, to be run BETWEEN fetching page one and page two. */
router.post("/kv-probe/del", async ({ request }) => {
  const puter = appPuter();
  if (!puter) return json({ ok: false, note: "no app puter" }, 500);

  const key = new URL(request.url).searchParams.get("key");
  if (typeof key !== "string" || !key.startsWith(PROBE_PREFIX)) {
    return json(
      { ok: false, note: `key must start with ${PROBE_PREFIX}` },
      400,
    );
  }
...
/** Puts the store back how it was found. */
router.post("/kv-probe/cleanup", async () => {
  const puter = appPuter();
  if (!puter) return json({ ok: false, note: "no app puter" }, 500);
  ...
  const items = Array.isArray(page) ? page : (page?.items ?? page?.keys ?? []);
  const keys = items.map((item) =>
    typeof item === "string" ? item : item?.key,
  );
  for (const key of keys) {
    if (typeof key === "string") await puter.kv.del(key);
  }
